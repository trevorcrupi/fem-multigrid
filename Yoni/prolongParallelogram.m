...
function P=prolongParallelogram(L)

model=createpde(1);
[a,b,c]=meshgeometryParallelogram(1);
g=decsg(a,b,c);
...
edge=edges(TR);
[N_edge,~]=size(edge);
load(['parallelogram/new_ele',num2str(L),'.mat']);
...
edge2=edges(TR2);
[N_edge2,~]=size(edge2);
temp=load(['parallelogram/new_ele',num2str(L+1),'.mat']);
