function error = getL2ErrorSquaredforProblem3(c,numOfTriangles,p,t,edge,iterationNum)
    ...
    for i = 1:numOfTriangles      % Calculate (complete L2-error)^2
        ...
        localU_hLine3 = @(r,z) ...
                      ...
                      + c(cAndRVector(4))*psiLine3(psi4,r) + c(cAndRVector(5))*psiLine3(psi5,r) + c(cAndRVector(6))*psiLine3(psi6,r);
        ...
        errorLine2 = U_theta(X,Y) - localU_hLine1(X,Y);
        errorLine3 = U_z(X,Y)     - localU_hLine1(X,Y);
        
        localL2ErrorSquared = Wx' * ( X.* [u(X,Y) - localU_h(X,Y)] ).^2 * Wy;
