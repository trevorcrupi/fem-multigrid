function solution = MGParallelogram(inputUVector,globalB, meshNum, storingA, storingEdge, storeNodeNums,storeHeights)
...
%         numOfMGIterations    = numOfMGIterations +1;
        ...
        if meshNum == 1 %Base case - ends recursion.
            ...
            % Step 1
            V1 = GSFunctionForMGParallelogram(storingA{meshNum}, globalB, storeHeights{meshNum}, storeNodeNums{meshNum}, inputUVector, meshNum);


            % Step 2
            zeroVector = zeros(storeHeights{meshNum-1},1);
            P          = prolongParallelogram(meshNum-1);
            newGlobalB = P'*(globalB - storingA{meshNum} * V1);
%                                 MG(inputUVector,globalB, meshNum, storingA, storingEdge, storeNodeNums,storeHeights)
            V2         = V1 + P * MGParallelogram( zeroVector, newGlobalB, meshNum-1, storingA, storingEdge, storeNodeNums, storeHeights);


            % Step 3
            V3       = GSFunctionTransposeParallelogram(storingA{meshNum}, globalB, storeHeights{meshNum}, storeNodeNums{meshNum}, V2, meshNum);
            solution = V3;
        end
