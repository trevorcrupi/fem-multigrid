...
function P=prolongLShapeD(L) 

model=createpde(1);
[a,b,c]=meshgeometryLShapeD(1);
g=decsg(a,b,c);
...
edge=edges(TR);
[N_edge,~]=size(edge);
% load(['LShapeDomain/new_eleL',num2str(L),'.mat']); %Had to replace.
mystr = ['LShapeDomain/new_eleL' num2str(L) '.mat'];
a = load(mystr);
...
edge2=edges(TR2);
[N_edge2,~]=size(edge2);
temp=load(['LShapeDomain/new_eleL',num2str(L+1),'.mat']);
